...
perc_atheist_us12 <- us12atheist/us12n
...
# Let's do Turkey, since I've lived there:

turkey12 <- atheism %>%
  filter(nationality == "Turkey" , year == "2012")

inference(y = response, data = turkey12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")

# ...and Canada, since it's a good comparable nation to the U.S.
...
inference(y = response, x = as.factor(year), data = spainDiff,
          method = "theoretical", type = "ci", 
          statistic = "proportion", success = "atheist")

inference(y = response, x = as.factor(year), data = spainDiff,
          method = "theoretical", type = "ht", alternative = "twosided", null = 0,
          statistic = "proportion", success = "atheist")
...
inference(y = response, x = as.factor(year), data = usDiff,
          method = "theoretical", type = "ht", alternative = "twosided", null = 0,
          statistic = "proportion", success = "atheist")
...
numCountries = 39
confLevel = 0.95
numType_One_Errors = numCountries * (1 - confLevel)
numType_One_Errors
...
me = 0.01      # margin of errro given as desired parameter
confLevel = 0.95     # confidence level of 95% also given
z_star = 1.96        # the Z* level associated with 95% confidence
se = me/z_star       # since margin of error = Z* x the Standard Error
p_safe = 0.5         # with the actual pop proportion unknown, we play it safe
                     # and use a est. proportion that maximizes our error level

# And since me = z_star*se = 1.96 * se
# and since se = sqrt(p * (1-p) / n), then

n = (p_safe * (1-p_safe))/(se^2)
